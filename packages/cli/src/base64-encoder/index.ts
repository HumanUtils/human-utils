...
export const encodeToBase64 = (text: string, options?: Base64Options): string => {
  try {
    ...
    throw new Error(
      `Failed to encode: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
};
...
export const decodeFromBase64 = (base64Text: string): string => {
  try {
    ...
    throw new Error(
      `Failed to decode: ${error instanceof Error ? error.message : 'Invalid Base64 string'}`
    );
  }
};
...
export const processBase64 = (input: string, operation: Base64Operation): Base64Result => {
  try {
    const output = operation === 'encode' ? encodeToBase64(input) : decodeFromBase64(input);
...
export const processBatch = (
  ...
  return lines.map((line) => {
    try {
      const output =
        operation === 'encode' ? encodeToBase64(line, options) : decodeFromBase64(line);
